...
const isUnsafeProductionSecret = (value: string) => {
  ...
  return (
    ...
    ['secret', 'change_me', 'changeme', 'replace-me', 'replace_with_a_secret'].includes(normalized) ||
  ...
  if (data.NODE_ENV === 'production') {
    if (isUnsafeProductionSecret(data.JWT_SECRET)) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
        path: ['JWT_SECRET'],
        message: 'JWT_SECRET must be a unique, non-placeholder value of at least 32 characters in production'
      });
    }
    ...
    if (isUnsafeProductionSecret(data.REFRESH_TOKEN_SECRET)) {
      context.addIssue({
        ...
        message: 'REFRESH_TOKEN_SECRET must be a unique, non-placeholder value of at least 32 characters in production'
      });
    }
