...
const booleanFromEnv = z.preprocess((value) => {
  ...
  if (typeof value === 'string') {
    ...
    const booleanLiteral = normalized.match(
      /(?:^|[^a-z])(true|false|1|0|yes|no|on|off)(?:$|[^a-z])/i,
    );
...
const isUnsafeProductionSecret = (value: string) => {
  ...
  return (
    ...
    ['secret', 'change_me', 'changeme', 'replace-me', 'replace_with_a_secret'].includes(
      normalized,
    ) ||
...
const envSchema = z
  .object({
    ...
    JWT_SECRET: requiredTrimmedStringFromEnv(1, 'JWT_SECRET should be at least 1 character long'),
    JWT_EXPIRES_IN: requiredTrimmedStringFromEnv().default('7d'),
    REFRESH_TOKEN_SECRET: requiredTrimmedStringFromEnv(
      1,
      'REFRESH_TOKEN_SECRET should be at least 1 character long',
    ),
    ...
    BREVO_API_KEY: optionalTrimmedStringFromEnv(),
    ...
    GOOGLE_OAUTH_CLIENT_ID: optionalTrimmedStringFromEnv(),
  ...
  .superRefine((data, context) => {
    ...
    const hasMailCredentials =
      Boolean(data.SMTP_USER) && Boolean(data.SMTP_PASS) && Boolean(data.MAIL_FROM);
    ...
    if (data.UPLOAD_PROVIDER === 'cloudinary') {
      ...
      if (!hasCloudinaryTuple) {
        context.addIssue({
          ...
          message:
            'Cloudinary uploads require CLOUDINARY_CLOUD_NAME, CLOUDINARY_API_KEY, and CLOUDINARY_API_SECRET',
        });
      }
    }
    ...
    if (data.NODE_ENV === 'production') {
      if (isUnsafeProductionSecret(data.JWT_SECRET)) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['JWT_SECRET'],
          message:
      ...
      if (isUnsafeProductionSecret(data.REFRESH_TOKEN_SECRET)) {
        context.addIssue({
          code: z.ZodIssueCode.custom,
          path: ['REFRESH_TOKEN_SECRET'],
          message:
            'REFRESH_TOKEN_SECRET must be a unique, non-placeholder value of at least 32 characters in production',
        });
      }
...
export const smtpConfig =
  env.SMTP_SERVICE || env.SMTP_HOST
    ? {
        ...
        service:
          env.SMTP_SERVICE ?? (env.SMTP_HOST?.toLowerCase() === 'smtp.gmail.com' ? 'gmail' : null),
...
export const hasBrevoConfig = Boolean(env.BREVO_API_KEY && env.BREVO_FROM);
export const hasResendConfig = Boolean(env.RESEND_API_KEY);
export const hasGoogleOAuthConfig = Boolean(env.GOOGLE_OAUTH_CLIENT_ID);
