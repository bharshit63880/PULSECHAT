...
export const ChatListItem = ({ chat, currentUserId, active, onClick }: ChatListItemProps) => {
  ...
  return (
    <button
      ...
      onClick={onClick}
      className={cn(
        'group relative flex w-full items-center gap-3 rounded-2xl border px-3 py-3 text-left transition-colors duration-150',
        active
          ? 'border-accent/25 bg-accent-soft shadow-sm'
          : 'border-transparent hover:border-line hover:bg-card-muted dark:hover:bg-slate-950/55'
      )}
    >
