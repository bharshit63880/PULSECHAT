...
## Message idempotency index migration

Production deployments must explicitly apply the sender-scoped message idempotency index migration:

```bash
npm run migrate:message-index -w @chat-app/api
```

The script is safe to run repeatedly. It removes the legacy global `clientMessageId_1` index, replaces outdated compound variants, and creates the unique partial `{ sender, clientMessageId }` index. The partial filter excludes messages without a client message id. To roll back, restore a known-good database backup; do not recreate the legacy global index because it can disclose messages across users.
